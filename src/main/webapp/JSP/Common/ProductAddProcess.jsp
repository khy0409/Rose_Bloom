...
<%
	...
	try{
		...
		int result = dao.Add_Product(productNum, productName, productDetail, summary, Integer.parseInt(price), Float.parseFloat(sale), shopId, category, imageURL);
		
		JSFunction js = new JSFunction();
		if(result == 1) {
			String link = "Product.jsp?ShopId=" + shopId;
			js.JSF(out, "상품 추가 완료", link);
		}else {
			String link = "ProductAdd.jsp?ShopId=" + shopId;
			js.JSF(out, "상품 추가 실패", link);
		}
	}catch(Exception e){
		e.printStackTrace();
	}
